...
const RecipeCategoryManager = () => {
  ...
  const fetchRecipes = async (categoryId) => {
    try {
      ...
      setRecipes(prev => ({ ...prev, [categoryId]: res.data }));
    } catch (err) {
  ...
  return (
    <div className="container">
      <h2 className="title"> Recipe Manager</h2>
        ...
        {categories.map((cat) => (
          <div key={cat._id} className="categoryCard">
            ...
            <ul className="recipeList">
              {recipes[cat._id] && recipes[cat._id].length > 0 ? (
                recipes[cat._id].map((recipe) => (
        ...
        <div className="modalOverlay" onClick={() => setSelectedRecipe(null)}>
          <div className="modalContent" onClick={(e) => e.stopPropagation()}>
            ...
            <section>
              <h4>Ingredients</h4>
              <ul>
                {selectedRecipe.ingredients.map((i, ind) => (
                  <li key={ind}>{i}</li>
                ))}
              </ul>
            </section>
            ...
            <section>
              <h4>Steps</h4>
              <ol>
                {selectedRecipe.steps.map((step, ind) => (
                  <li key={ind}>{step}</li>
                ))}
              </ol>
            </section>
